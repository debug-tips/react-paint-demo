(function (global, factory) {
  ...
  (global.PaintDemo = factory(global.React));
  ...
  function (_Component) {
    ...
    function App(props) {
      ...
      _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "updateTimer", function (stopTime) {
        requestAnimationFrame(function () {
          ...
          if (stopTime > newTime) {
            ...
            var _this$getMetrics = _this.getMetrics(),
                finalFcp = _this$getMetrics.finalFcp,
                finalFmp = _this$getMetrics.finalFmp,
                finalLoad = _this$getMetrics.finalLoad,
                finalFp = _this$getMetrics.finalFp;

            var arr = [finalFcp, finalFmp, finalFp, finalLoad].sort(function (a, b) {
              return a - b;
            });
            arr.pop();
            var isMetStopPoint = !arr.every(function (t) {
              return Math.abs(newTime / _this.state.slowFactor - t) > FPS;
            });

            if (isMetStopPoint) {
              _this.setState({
                time: newTime + FPS * _this.state.slowFactor
              });
      ...
      _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "playAnimation", function () {
        var _this$getMetrics2 = _this.getMetrics(),
            finalFcp = _this$getMetrics2.finalFcp,
            finalFmp = _this$getMetrics2.finalFmp,
            finalLoad = _this$getMetrics2.finalLoad,
            finalFp = _this$getMetrics2.finalFp;
      ...
      _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "renderContentByTime", function (time) {
        var pageTitle = _this.props.pageTitle;

        var _this$getMetrics3 = _this.getMetrics(),
            finalFcp = _this$getMetrics3.finalFcp,
            finalFmp = _this$getMetrics3.finalFmp,
            finalLoad = _this$getMetrics3.finalLoad,
            finalFp = _this$getMetrics3.finalFp;
    ...
    _createClass(App, [{
      ...
      value: function render() {
        ...
        var _this$getMetrics4 = this.getMetrics(),
            ...
            finalFmp = _this$getMetrics4.finalFmp,
            finalLoad = _this$getMetrics4.finalLoad,
            finalFp = _this$getMetrics4.finalFp;
